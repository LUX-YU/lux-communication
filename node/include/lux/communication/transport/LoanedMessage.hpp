...
namespace lux::communication { template<typename T> class Publisher; }
...
public:
    ...
    void commit(uint32_t total_size) {
        if (!msg_ || committed_) return;
        writer_->commitSlot(total_size);
        committed_ = true;
    }

private:
    template<typename U> friend class ::lux::communication::Publisher;
