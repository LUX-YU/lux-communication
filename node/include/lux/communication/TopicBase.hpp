...
namespace lux::communication
{
    ...
    public:
        ...
        {
            return std::atomic_load_explicit(&sub_snapshot_, std::memory_order_acquire);
        }
    ...
    protected:
        ...
        void setIdInDomain(size_t index)
        {
            id_in_domain_ = index;
        }
        ...
        /**
         * @brief Rebuild the subscriber snapshot.
         *        Must be called while holding mutex_sub_.
         */
        ...
        void foreachSubscriber(Func&& func)
        {
            ...
            auto snapshot = std::atomic_load_explicit(&sub_snapshot_, std::memory_order_acquire);
            if (!snapshot)
            {
                return;
            }
    ...
    protected:
        ...
        // GCC 11/libstdc++ 11 下不要使用 std::atomic<std::shared_ptr<T>>
        SubscriberSnapshot sub_snapshot_;
