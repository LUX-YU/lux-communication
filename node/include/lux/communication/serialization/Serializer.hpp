...
constexpr transport::SerializationFormat selectSerializationFormat() {
    ...
    if constexpr (TriviallyCopyableMsg<T>)   return RawMemcpy;
    else if constexpr (ProtobufMessage<T>)   return Protobuf;
    else if constexpr (CustomSerializable<T>) return Custom;
