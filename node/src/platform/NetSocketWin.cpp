...
UdpSocket& UdpSocket::operator=(UdpSocket&& o) noexcept {
    if (this != &o) { close(); sock_ = o.sock_; o.sock_ = kInvalidSocket; }
    return *this;
}
...
int UdpSocket::recvFrom(void* buf, size_t max_len,
    ...
    if (n < 0) {
        ...
        if (err == WSAEWOULDBLOCK) return 0;  // non-blocking, no data
        return -1;
    }
...
{
    o.sock_ = kInvalidSocket; o.remote_port_ = 0;
}
...
TcpSocket TcpListener::accept(std::string& remote_addr, uint16_t& remote_port) {
    if (sock_ == kInvalidSocket) return TcpSocket{};

    sockaddr_in sa{};
    int sa_len = sizeof(sa);
    SOCKET client = ::accept(static_cast<SOCKET>(sock_),
                             reinterpret_cast<sockaddr*>(&sa), &sa_len);
    if (client == INVALID_SOCKET) return TcpSocket{};
