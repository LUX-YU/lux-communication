    ...
    : domain_id_(domain_id)
{
    ...
    size_t total_size = sizeof(ShmRegistryHeader)
                      + max_entries * sizeof(TopicEndpointEntry);
...
int32_t ShmRegistry::findFreeSlot() const {
    ...
    for (uint32_t i = 0; i < max; ++i) {
        if (entryAt(i)->status.load(std::memory_order_relaxed)
                == static_cast<uint32_t>(EntryStatus::Free)) {
...
void ShmRegistry::withdraw(int32_t slot_index) {
    if (slot_index < 0
        || static_cast<uint32_t>(slot_index) >= header()->max_entries)
        return;
...
void ShmRegistry::withdrawAll(uint32_t pid) {
    ...
    for (uint32_t i = 0; i < max; ++i) {
        auto* e = entryAt(i);
        if (e->status.load(std::memory_order_relaxed)
                == static_cast<uint32_t>(EntryStatus::Active)
            && e->pid == pid) {
...
std::vector<LookupResult> ShmRegistry::lookup(const LookupFilter& filter) const {
    ...
    for (uint32_t i = 0; i < max; ++i) {
        ...
        if (e->topic_name_hash != name_hash)                           continue;
        if (e->role != static_cast<uint8_t>(filter.role))              continue;
        if (filter.type_hash != 0 && e->type_hash != filter.type_hash) continue;
