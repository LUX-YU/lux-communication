...
ShmDataPool::AllocResult ShmDataPool::allocate(uint32_t size, uint32_t ref_count) {
    ...
    const uint32_t min_split = static_cast<uint32_t>(sizeof(BlockHeader))
                             + header_->min_block_size;
    ...
    while (cur_offset != 0) {
        ...
        if (block->total_size >= needed) {
            ...
            const uint64_t rc_offset = cur_offset
                + offsetof(BlockHeader, ref_count);
