...
FragmentAssembler::feed(const FragmentHeader& fh, const void* payload, size_t len) {
    if (fh.frag_magic != kFragmentMagic) return std::nullopt;
    if (fh.seq_in_group >= fh.total_fragments) return std::nullopt;
    if (fh.total_msg_size > kMaxFragmentedMsgSize) return std::nullopt;
