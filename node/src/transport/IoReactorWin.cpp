...
struct IoReactor::Impl {
    ...
    bool postZeroRead(FdContext* ctx) {
        if (ctx->read_pending || ctx->closing || ctx->needs_poll) return false;
    ...
    bool postZeroWrite(FdContext* ctx) {
        if (ctx->write_pending || ctx->closing || ctx->needs_poll) return false;
    ...
    bool addFd(platform::socket_t fd, uint8_t events, EventCallback cb) {
        ...
        if (!raw->needs_poll) {
            if (events & Readable)  postZeroRead(raw);
            if (events & Writable)  postZeroWrite(raw);
        }
        return true;
    }
    ...
    bool modifyFd(platform::socket_t fd, uint8_t events) {
        ...
        if (!ctx->needs_poll) {
            if ((events & Readable) && !(old & Readable))  postZeroRead(ctx);
            if ((events & Writable) && !(old & Writable))  postZeroWrite(ctx);
        }
        return true;
    }
    ...
    int dispatchIOCP(OVERLAPPED_ENTRY* entries, ULONG count) {
        ...
        for (ULONG i = 0; i < count; ++i) {
            if (entries[i].lpCompletionKey == kWakeupKey) continue;
...

            {
                ...
                if (it != fds_.end()) {
                    ...
                    if (evt == Readable)  ctx->read_pending  = false;
                    if (evt == Writable)  ctx->write_pending = false;
                    ...
                    for (auto git = graveyard_.begin(); git != graveyard_.end(); ++git) {
                        auto* gctx = git->get();
                        if (gctx->fd != fd) continue;
                        if (evt == Readable)  gctx->read_pending  = false;
                        if (evt == Writable)  gctx->write_pending = false;
            ...
            if (rearm_read || rearm_write) {
                ...
                if (it != fds_.end() && !it->second->closing) {
                    if (rearm_read)  postZeroRead(it->second.get());
                    if (rearm_write) postZeroWrite(it->second.get());
                }
            }
        }
        return dispatched;
    ...
    int dispatchPollFds(int timeout_ms) {
...

        {
            std::lock_guard lock(mutex_);
            for (auto& [fd, ctx] : fds_) {
                if (!ctx->needs_poll || ctx->closing) continue;
                WSAPOLLFD pfd{};
                pfd.fd = static_cast<SOCKET>(fd);
                if (ctx->events & Readable) pfd.events |= POLLIN;
                if (ctx->events & Writable) pfd.events |= POLLOUT;
                pollfds.push_back(pfd);
                poll_keys.push_back(fd);
            }
        }

        if (pollfds.empty()) return 0;
        ...
        for (size_t i = 0; i < poll_keys.size(); ++i) {
            ...
            uint8_t fired = 0;
            if (pollfds[i].revents & POLLIN)                fired |= Readable;
            if (pollfds[i].revents & POLLOUT)               fired |= Writable;
            if (pollfds[i].revents & (POLLERR | POLLHUP))   fired |= Error;
            if (!fired) continue;
            ...
            EventCallback cb_copy;
            {
                ...
                if (it == fds_.end() || it->second->closing) continue;
    ...
    bool hasPollFds() const {
        for (auto& [_, ctx] : fds_)
            if (ctx->needs_poll && !ctx->closing) return true;
        return false;
    }
