...
void Executor::addNode(std::shared_ptr<intraprocess::Node> node)
{
    if (!node) return;
...
void Executor::removeNode(std::shared_ptr<intraprocess::Node> node)
{
    if (!node) return;
    auto groups = node->getCallbackGroups();
    std::lock_guard<std::mutex> lock(mutex_);
...
void TimeOrderedExecutor::addNode(std::shared_ptr<intraprocess::Node> node)
{
    if (!node) return;
