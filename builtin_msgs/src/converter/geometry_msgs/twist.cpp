...
namespace lux::communication::builtin_msgs
{
	...
	template<> LUX_COMMUNICATION_PUBLIC void pb_st_converter::pb2st(const PBTwist& in, STTwist& out)
	{
		pb_st_converter::pb2st(in.linear(), out.linear);
		pb_st_converter::pb2st(in.angular(), out.angular);
	}

	template<> LUX_COMMUNICATION_PUBLIC void pb_st_converter::st2pb(const STTwist& in, PBTwist& out)
	{
